...
NIFTY_CANDLE_BREAKOUT_V2 — C1 point-move trigger + C2 point-move entry, Nifty 50 only.

REQUIREMENTS (as given — rewritten 2026-07-20, replaces the old
10s-body / 5s-confirm-color / breakout-watch design entirely)
──────────────────────────────────────────────────────────────
  1. Nifty only.
  2. C1: watch every tick within the current 10-second window against
     THAT window's own open. If price moves >= c1_move_pts (6 points)
  ...
  3. C2: the moment C1 closes, start a new window with open = C1's
     ...
     Close C2 immediately if price moves >= c2_move_pts (3 points) from
  ...
  6. SL/TP are fixed at 10 points on the OPTION PREMIUM (not the index),
...
CFG = {
    ...
    "sl_points"              : 12.0,
    "trail_activate_pts"     : 10.0,  # open profit before trail arms
    ...
    # This strategy's ONLY entry mode is c1_c2_points_entry. Across the full
    # entry-paired history (2026-07-14 -> 2026-08-26) that mode is
    # -7,997 over 381 trades (-21/trade) and negative in BOTH months
    # independently (Jul -4,531/173, Aug -3,466/208). Gross is roughly +12k;
    # the ~52 Rs/trade cost drag on 381 trades is what turns it negative — it
    ...
    # ── Output ────────────────────────────────────────────────────────────────
    "csv_file"               : "nifty_candle_breakout_v2_trades.csv",
}


class NiftyCandleBreakoutV2Strategy(BaseStrategy):
    ...
    @property
    def name(self) -> str:
        return "NIFTY_CANDLE_BREAKOUT_V2"
    ...
    def _indicator_snapshot(self, spot: float) -> dict:
        ...
        # FIX (2026-08-26): hub.session_vwap is fed ONLY by the BankNifty
        # index tick, so this used to compare Nifty spot (~24,300) against a
        # BankNifty VWAP (~57,500) — spot_vs_vwap was "BELOW" on 73 of 73
        # logged rows. With enforce_direction_gate=True that would have
        # blocked 100% of CE entries and passed 100% of PE. Use the Nifty one.
        vwap    = self._hub.nifty_vwap.value
    ...
    def _fire_entry(self, signal: str, index_price: float, ts: datetime):
        ...
        self._build_entry(sym, token, signal, ts, reason="c1_c2_points_entry")
    ...
    def _start_emergency_exit(self, t: dict, ref_price: float, reason: str, ts: datetime):
        ...
        thread = threading.Thread(
            target=_loop, name="nifty-candle-breakout-v2-emergency-exit", daemon=True,
        )
        thread.start()
