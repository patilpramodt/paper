"""
strategies/stock_options_scanner_realtime_strategy.py

STOCK_OPT_SCANNER_RT — multi-stock option buyer, REAL-TIME VOLUME-SURGE ENTRY.

This is a NEW, separate strategy. V1 (stock_options_scanner_strategy.py) is
left completely unmodified and keeps running unchanged as the reference
implementation with its original bar-close entry. Only the ENTRY LOGIC
differs here — universe, liquidity gates, sizing and the exit ladder are
the same mechanisms V1 uses after its 2026-08-28 fixes.
...
═══════════════════════════════════════════════════════════════════════════
  ENTRY LOGIC — real-time volume surge, evaluated on EVERY tick
═══════════════════════════════════════════════════════════════════════════
  V1 waits for a 3-minute bar to CLOSE before checking anything — a move
  starting mid-bar isn't acted on for up to 3 minutes. This strategy checks
  two conditions on every incoming tick, against a rolling window, so it
  ...
  candle to finish. That is what makes them LEADING rather than lagging;
  stochastic/RSI/MACD are all built from closes and by construction turn
  only after price has already turned.

  confirm_mode="one_confirm" (default) needs a second same-direction
  sample within 20s before it acts — filters a single noisy print at the
  cost of a few seconds. See the worked example below.
...
log = logging.getLogger("strategy.stock_opt_scanner_rt")
...
#              the signal fired — the price of the very tick that triggered
#              the surge signal ("last tick high/low")
...
class _StockState:
    ...
    __slots__ = (
        ...
        "ticks", "surge_pending", "last_surge_ts",
    )
    ...
    def __init__(self, sym: str, token: int):
        ...
        # Tick ring buffer for the real-time surge detector: (ts, price, dvol).
        self.ticks           = deque(maxlen=4000)
        self.surge_pending   = None
        self.last_surge_ts   = None
    ...
    # ── real-time surge measurement ──────────────────────────────────────────

    def atr_or_est(self, n: int) -> Optional[float]:
        a = self.atr(n)
        if a:
            return a
        if len(self.bars) >= 3:
            return self.atr(len(self.bars))
        return None

    def window_stats(self, ts, window_sec: int):
        if not self.ticks:
            return None, None, None
        cutoff = ts - timedelta(seconds=window_sec)
        vol = 0
        first_price = None
        for t, px, dv in self.ticks:
            if t < cutoff:
                continue
        ...
        if first_price is None:
            return None, None, None
        now_price = self.ticks[-1][1]
        if self.ticks[0][0] > cutoff:
            return None, None, None
        roc = ((now_price - first_price) / first_price * 100.0) if first_price else 0.0
        return vol, roc, now_price

    def baseline_window_volume(self, window_sec: int, bar_minutes: int, n_bars: int):
        avg_bar = self.avg_volume(n_bars)
        if not avg_bar:
            return None
        return avg_bar * (window_sec / float(bar_minutes * 60))


class StockOptionsScannerRealtimeStrategy(BaseStrategy):
    """
    ...
    def _on_spot_tick(self, st: _StockState, price: float, ts: datetime, tick_ts: datetime):
        ...
        st.ticks.append((tick_ts, price, dv))
        cutoff = tick_ts - timedelta(seconds=CFG["surge_window_sec"] * 3)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # LEADING SIGNAL — real-time volume surge (runs on every spot tick)
    # ══════════════════════════════════════════════════════════════════════════

    def _evaluate_surge(self, st: _StockState, price: float, ts: datetime, tick_ts: datetime):
        """
        Two conditions, both measured on the live tick stream — no bar close,
        no smoothing:

          1. VOLUME SURGE   volume in the last surge_window_sec is
                            surge_mult x the stock's own normal pace for a
                            window that length
          2. PRICE VELOCITY |ROC| over the same window >= min_roc_pct; its
                            SIGN picks CE (up) or PE (down)

        confirm_mode="one_confirm" (default) holds the first qualifying
        sample and requires a second, same-direction sample within
        confirm_window_sec before entering — filters single-print spikes at
        the cost of a second or two. "immediate" fires on the first sample.
        """
        t = ts.time()
        ...
        if len(st.bars) < CFG["min_bars"]:
            return
        if st.last_surge_ts and \
                (ts - st.last_surge_ts).total_seconds() < CFG["surge_cooldown_sec"]:
            return

        win = CFG["surge_window_sec"]
        vol, roc, now_px = st.window_stats(tick_ts, win)
        if vol is None:
            return
        base = st.baseline_window_volume(win, CFG["bar_minutes"], CFG["vol_avg_bars"])
        if not base:
            return

        ratio = vol / base
        if ratio < CFG["surge_mult"]:
            st.surge_pending = None
            return
        if abs(roc) < CFG["min_roc_pct"]:
            return

        side = "UP" if roc > 0 else "DOWN"

        if CFG["require_breakout"]:
            ph = st.prior_high(CFG["breakout_lookback"])
            pl = st.prior_low(CFG["breakout_lookback"])
            if side == "UP" and (ph is None or price < ph):
                return
            if side == "DOWN" and (pl is None or price > pl):
                return
        if CFG["require_vwap_align"]:
            vwap = st.vwap()
            if vwap:
                if side == "UP" and price < vwap:
                    return
                if side == "DOWN" and price > vwap:
                    return

        meta = {"close": price, "vol_ratio": ratio, "roc": roc, "vwap": st.vwap()}

        if CFG["confirm_mode"] == "one_confirm":
            pend = st.surge_pending
            if pend is None or pend["side"] != side or \
                    (ts - pend["ts"]).total_seconds() > CFG["confirm_window_sec"]:
                st.surge_pending = {"side": side, "ts": ts, "ratio": ratio, "roc": roc}
                self._log_signal(ts, st.sym, "SURGE_PENDING", side, **meta)
                return
            st.surge_pending = None

        st.last_surge_ts = ts
        self._log_signal(ts, st.sym, "SURGE_TRIGGER", side, **meta)
        log.info(
        ...
        if atr is None:
            self._log_signal(ts, st.sym, "NO_ATR", side, block="insufficient_bars")
            return
        self._start_confirm(st, side, price, atr, ts, meta)
    ...
    def _start_confirm(self, st: _StockState, side: str, spot: float,
        ...
        the very tick that triggered the surge — is kept as the reference
        ("last tick high/low"). From this instant, _CONFIRM_CANDLE_SEC-
        second candles are built off the live tick stream (own clock —
        bucket 0 starts exactly at `ts`, not the wall-clock second grid).
        The first such candle that both (a) closes the signal's own color
        and (b) closes beyond `spot` calls _arm_entry(). If none does
        within _CONFIRM_TIMEOUT_S seconds, the signal is dropped and no
        trade is taken.
        """
    ...
    def _arm_entry(self, st: _StockState, side: str, spot: float,
        ...
        log.info(
            f"[{self.name}] {sym} {side} surge @ {spot:.2f} "
            f"(vol x{meta.get('vol_ratio', 0):.1f}, ROC {meta.get('roc', 0):+.2f}%) "
